...
// (addCard, removeLastCard, resetAll, calculateTotal, updateCardList, saveResult は変更なしのため省略)
function addCard() {
    ...
    for (const str of cardStrings) {
        ...
        if (isNaN(val) || val < 1 || val > 12) {
            errorMsg.textContent = `無効な入力 "${str.trim()}" が含まれています。1〜12の数字をカンマ区切りで入力してください。`;
            return;
        }
        cardsToAdd.push(val);
    }
    ...
    for (const val of cardsToAdd) {
        if (val === 6) {
            ...
            if (lastCard !== 5) {
                errorMsg.textContent = "カード6はカード5の直後にしか使えません。入力を中断しました。";
                updateCardList();
                updateLiveResult();
                return;
            }
...
function findImprovedSolution(baseOrder, basePoint) {
    ...
    for (let i = 0; i < 200; i++) {
        ...
        if (action < 0.5) {
            ...
            const newCard = ALL_CARDS[Math.floor(Math.random() * ALL_CARDS.length)];
